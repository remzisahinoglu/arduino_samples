...
String bilgi = "";                                                        // istasyon datası karakterlerinin birleştirildiği string
...
unsigned int cutIstasyon = 0;                                             // datadaki istasyon bilgisinin kırpılması için
...
void setup()
{
  ...
  digitalWrite(S0, HIGH);digitalWrite(S1, HIGH);digitalWrite(S2, HIGH);digitalWrite(S3, HIGH);digitalWrite(S4, HIGH);
  digitalWrite(S5, HIGH);digitalWrite(S6, HIGH);digitalWrite(S7, HIGH);
  delay(1000);
...
void loop() 
{
  ...
  while (Serial2.available())                                             // bilgisayardan data geldiğinde
  {
    ...
    cutIstasyon = 1;                                                      // pc den gelen bilgiyi istasyona göndermek için
  }
  
  if(cutIstasyon == 1)                                                    // pc den gelen dataları istasyondan gönder
  {
    cutIstasyon = 0;                                                      // sürekli göndermemek için sıfırlandı
                                                                          // 
    for (int i = 0; i < 4; i++)                                           // ilk gelen paket istasyon numarasını temsil ettiğinden onu kırp
    {                                                                     // demuxu ayarla gerisini istasyona gönder
      char ch = inputString_pc[i];                                        // sırayla tek tek karakterleri oku
      if (ch == ' ')                                                      // karakterler arasında boşluk olduğunda okunan datayı kaydet
      {
        istasyon = bilgi.toInt();
        demux();
        sendtoIstasyon == 1;
        break;
      }
  ...
  if(sendtoIstasyon == 1)
  {
    ...
    Serial1.flush();
    inputString_pc = "";                                                  // bir sonraki gelen datada karışılığı engellemek için
  }
